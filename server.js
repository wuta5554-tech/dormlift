...
// --- 3. PostgreSQL Connection (FIX FOR ECONNREFUSED) ---
const pool = new Pool({
  // Railway provides DATABASE_URL. If missing, it fails correctly instead of defaulting to localhost.
  connectionString: process.env.DATABASE_URL,
  ssl: {
    // REQUIRED for Railway and most cloud DB providers
...
app.post('/api/auth/send-code', async (req, res) => {
    ...
    verificationCodes.set(email, { code, expires: Date.now() + 600000 }); // 10 min expiry
        ...
        await transporter.sendMail({
            ...
            html: `
                <div style="font-family:sans-serif;padding:20px;border:1px solid #3498db;border-radius:12px;">
                    ...
                    <p>Kia Ora! Use this code to join the community:</p>
                    <h1 style="background:#f4f7f9;padding:20px;text-align:center;letter-spacing:5px;color:#2c3e50;">${code}</h1>
                </div>`
        });
...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        ...
        res.status(400).json({ success: false, message: "Registration failed (ID may already exist)." }); 
    }
});
